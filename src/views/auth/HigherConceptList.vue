<template>
  <div class="higher-concept-list">
    <SideMenu />
    <div class="content">
      <div class="page-header">
        <h1>
          <i class="el-icon-data-line page-title-icon"></i>
          创新高概念分析
        </h1>
...
import HigherConceptStats from '../../components/HigherConceptStats.vue'
...
export default {
  ...
  methods: {
    ...
    async fetchConceptStats() {
      try {
        const response = await axios.get(`${config.apiBaseUrl}/higher-concept-stats/list`, {
          params: {
            tradeDate: this.selectedDate
          }
        })
        console.log('Higher Concept Stats Response:', response)
        
        if (response.data.code === 0) {
          this.conceptStats = response.data.data.map(item => ({
            conceptName: item.conceptName,
            stockCount: item.stockCount,
            avgGain: Number(item.avgGain),
            maxGain: Number(item.maxGain),
